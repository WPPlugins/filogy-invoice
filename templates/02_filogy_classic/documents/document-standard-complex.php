...
<?php 
wsl_log(null, '02 document-standard-complex.php BEFORE GET TEMPLATE: ' . wsl_vartotext(''));
wsl_log(null, '02 document-standard-complex.php $filo_document_templates: ' . wsl_vartotext($filo_document_templates));
wsl_log(null, '02 document-standard-complex.php $standard_template_key: ' . wsl_vartotext($standard_template_key));

wsl_log(null, '02 document-standard-complex.php $filo_document_templates[$standard_template_key][template_path]: ' . wsl_vartotext($filo_document_templates[$standard_template_key]['template_path']));
wsl_log(null, '02 document-standard-complex.php $filo_document_templates[$standard_template_key][default_path]: ' . wsl_vartotext($filo_document_templates[$standard_template_key]['default_path']));

?>
	<?php wc_get_template( 'documents/document-seller-customer.php', 
							array( 
								'order' 		=> $order,
	...
	<?php wc_get_template( 'documents/document-head-data-horizontal.php', 
							array( 
								'order' => $order,
								'doc_financial_data'	=> $doc_financial_data,
								'actual_template_key' => $actual_template_key,
	...
	<?php wc_get_template( 'documents/document-items_header.php', 
							array( 
								...
								'doc_financial_data'	=> $doc_financial_data,
								'actual_template_key' => $standard_template_key, // not $actual_template_key, because we call the standard template, and thus the actual template will be the standard template in the subsequent template files
								'standard_template_key' => $standard_template_key,
								'filo_document_templates' => $filo_document_templates, 
							),
							$filo_document_templates[$standard_template_key]['template_path'], //FILO()->template_path(), 
							$filo_document_templates[$standard_template_key]['default_path'] //FILO()->plugin_path() . '/templates/' //$default_path until includes subdir, this is important to locate WC it not from WC own plugin path
		);
	...
	<?php
		wc_get_template( 'documents/document-payment-and-shipping.php', 
						array(
							'order'                 => $order,
							'actual_template_key' => $actual_template_key,
							'standard_template_key' => $standard_template_key,
